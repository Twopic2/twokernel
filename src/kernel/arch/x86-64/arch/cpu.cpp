...
namespace Cpu {
    ...
    void init_syscall() {
        write_msr(MSRKGSBASE, reinterpret_cast<std::uint64_t>(&bsp_cpu));
        ...
        Util::klog("syscall: STAR=0x%llx (syscall cs 0x%llx)\n",
                   star, (star >> 32) & 0xFFFF);
    ...
    void init_cpu() {
        bsp_cpu.syscall_kernel_rsp = x86::System::Gdt::tss.rsp0;
    }
}
